    ...
    $ionicPlatform.ready(function () {
        ...
        $scope.loginWithGoogle = function () {
            ...
            $cordovaOauth.google(GOOGLEKEY, GOOGLEAUTHSCOPE).then(function (result) {
                ...
                alert("dada");
                //modify
                FBFactory.auth().$signInWithCredential(provider.getCredential(result.id_token, result.access_token)).then(function (authData) {
...
.controller('DashCtrl', ['$scope', 'UserFactory','$ionicPlatform', '$state', '$ionicHistory', function ($scope, UserFactory, $ionicPlatform, $state, $ionicHistory) {
    $ionicPlatform.ready(function () {
        ...
        $scope.redir = function (user) {
            $state.go('chat-detail', { otherUser: user });
        }
    });
}
])
                    ...
                    if (index === 0) {
                        ...
                        .then(function(results) {
                            if (results.length > 0) {
                                ...
                                {
                                    //Process the image
                                    string.postMessage('<p>' + $scope.user.cachedUserProfile.name + ' posted : <br/><img class="chat-img" src="' + base64Img + '">', 'img');
                                    Loader.hide();
                                });
                            }
                        }, function(error) {
